...
def main():
    ...
    print("ref depth mean:", np.mean(ref_depth[ref_depth > 0]))
    for imgname, depthname in zip(image_files, depth_files):
        ...
        print("frame depth mean:", np.mean(depth[depth > 0]))
...
def backproject(keypoints, depth, K):
    ...
    for i, kp in enumerate(keypoints):
        u, v = int((kp.pt[0]) // 1), int((kp.pt[1]) // 1)
        h, w = depth.shape
...
def ransac(ref_pts, frame_pts):
    ...
    for i in range(iterations):
        ...
        current_inliers_mask = errors < 0.2
